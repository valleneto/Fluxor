...
saida = Path.home() / "Projetos/INVENTARIOS/google_organizado.csv"
...
with open(saida, "w", newline="", encoding="utf-8") as csvfile:
    ...
    writer.writerow(["arquivo", "tamanho_mb", "hash", "caminho"])
    ...
    for arquivo in origem.rglob("*"):
        if arquivo.is_file() and arquivo.suffix in extensoes:
            ...
            writer.writerow([arquivo.name, tamanho, h, str(arquivo)])
            print(f"SCAN: {arquivo.name}")

print("\nINVENTÁRIO GOOGLE FINALIZADO.")
